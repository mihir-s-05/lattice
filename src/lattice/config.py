...
class RunConfig:
    ...
    def to_public_dict(self) -> Dict[str, Any]:
        ...
        out["command_policy"] = asdict(self.command_policy)
...
class ConfigurationFactory:
    ...
    def create_provider_configs(cls) -> Dict[str, ProviderConfig]:
        ...
        providers["openai"] = ProviderConfig(
            name="openai",
            base_url=cls._get_env("OPENAI_BASE_URL", "https://api.openai.com/v1"),
            api_key=cls._get_env("OPENAI_API_KEY"),
            model=cls._get_env("OPENAI_MODEL", DEFAULT_MODEL_BY_PROVIDER["openai"])
        )
    ...
    def create_system_limits(cls) -> SystemLimits:
        return SystemLimits(
            ...
            http_timeout=cls._get_env_int("LATTICE_HTTP_TIMEOUT", DEFAULT_HTTP_TIMEOUT),
            connect_timeout=cls._get_env_int("LATTICE_CONNECT_TIMEOUT", DEFAULT_CONNECT_TIMEOUT),
            retry_count=cls._get_env_int("LATTICE_RETRY_COUNT", DEFAULT_RETRY_COUNT),
            max_retry_delay=cls._get_env_int("LATTICE_MAX_RETRY_DELAY", DEFAULT_MAX_RETRY_DELAY),
        )
    ...
    def create_run_config(cls, run_id: str, prompt: str = "") -> RunConfig:
        ...
        rc = RunConfig(
            ...
            command_policy=cls.create_command_policy(),
        )
    ...
    @classmethod
    def _user_config_path(cls) -> str:
        return os.environ.get("LATTICE_USER_CONFIG") or os.path.join(os.path.expanduser("~"), ".lattice", "config.json")
    ...
    @classmethod
    def save_user_config(cls, data: Dict[str, Any]) -> str:
        path = cls._user_config_path()
    ...
    @classmethod
    def create_command_policy(cls) -> CommandPolicy:
        cfg = cls.load_user_config()
        policy = cfg.get("command_policy", {}) if isinstance(cfg, dict) else {}
        allowlist = list(policy.get("allowlist") or [])
        denylist = list(policy.get("denylist") or [])

        env_allow = cls._get_env("LATTICE_COMMAND_ALLOWLIST")
        env_deny = cls._get_env("LATTICE_COMMAND_DENYLIST")
        if env_allow is not None:
            allowlist = [x.strip() for x in env_allow.split(",") if x.strip()]
        if env_deny is not None:
            denylist = [x.strip() for x in env_deny.split(",") if x.strip()]
        return CommandPolicy(allowlist=allowlist, denylist=denylist)
